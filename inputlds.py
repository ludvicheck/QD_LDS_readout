...
class dynamical_system(object):
    ...
    def solve(self, h0, inputs, T, **kwargs):
        ...
        if self.n == 1:
            try:
                self.inputs = list(
                    np.squeeze(np.array(inputs, dtype=float).reshape(1, T))
                )
            except:
                print(
                    "Something wrong with inputs. Should be list of scalars of length %d."
                    % (T)
                )
                exit()
        else:
            ...
            if self.inputs.shape[0] != self.n or self.inputs.shape[1] != T:
                print(
                    "Something wrong with inputs: wrong dimension or wrong number of inputs."
                )
                exit()

        if str(self.process_noise).lower() == "gaussian":
            process_noise = np.matrix(
                np.random.normal(loc=0, scale=self.proc_std, size=(self.d, T))
            )
        else:
            process_noise = np.matrix(np.zeros((self.d, T)))

        if str(self.observation_noise).lower() == "gaussian":
            observation_noise = np.matrix(
                np.random.normal(loc=0, scale=self.proc_std, size=(self.m, T))
            )
        else:
        ...
        for t in range(T):
            ...
            if (
                (self.corrupt_probability is not None)
                and np.random.random_sample() <= self.corrupt_probability
                and t > earliest_event_time
            ):
...
class time_series(object):
    def __init__(
        self, matlabfile="./OARIMA_code_data/data/setting6.mat", varname="seq_d0"
    ):
        f = None
        self.outputs = []
        try:
    ...
    def solve(self, h0=[], inputs=[], T=100, **kwargs):
        ...
        if not isinstance(T, int):
            print(
                "T must be an integer. Anything less than 1 suggest no truncation is needed."
            )
            exit()
